...
# Training function.
def train(
    model, trainloader, optimizer, scaler,
    criterion, scheduler=None, epoch=None, device=torch.device('cuda')
):
    model.train()
    print('Training')
    ...
    for i, data in tqdm(enumerate(trainloader), total=len(trainloader)):
        ...
        optimizer.zero_grad()
        # Forward pass.
        with autocast(): # Using the autocast context manager for mixed precision training.
            outputs = model(image)
            # Calculate the loss.
        ...
        # Update the weights.
        # optimizer.step()
        scaler.step(optimizer) # Using the scaler instead of the optimizer.
        scaler.update()
...
# Validation function.
def validate(model, testloader, criterion, class_names, device=torch.device('cuda')):
    model.eval()
    print('Validation')
